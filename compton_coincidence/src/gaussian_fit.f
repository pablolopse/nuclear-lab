...
C
C     Steps:
C       1. Read the spectrum (one count per line = one channel).
...
C
C     Reads: ../data/calibration-2.txt (single-column spectrum).
C=======================================================================
      PROGRAM MAIN
      IMPLICIT NONE
...
C     --- Read the spectrum (channel, counts) to end of file ---
      open(10, File="../data/calibration-2.txt")
      NPTS=0
      DO i=1,NCHAN
        read(10,*,iostat=IOS) Y(i)
        if (IOS.ne.0) goto 5
        X(i)=i
        if (Y(i).gt.1.0) then
          SIG(i)=sqrt(Y(i))
